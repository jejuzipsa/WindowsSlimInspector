...
public partial class MainWindow : Window
{
    ...
    private void BuildFeatureRows()
    {
        ...
        foreach (var feature in _features)
        {
            ...
            var textStack = new StackPanel { Margin = new Thickness(12, 0, 10, 0) };
            textStack.Children.Add(new TextBlock { Text = feature.Name, FontWeight = FontWeights.SemiBold, Foreground = Brushes.WhiteSmoke });
            textStack.Children.Add(new TextBlock { Text = feature.Description, Foreground = new SolidColorBrush(Color.FromRgb(170, 182, 199)), TextWrapping = TextWrapping.Wrap });
            Grid.SetColumn(textStack, 1);

            var status = new TextBlock { VerticalAlignment = VerticalAlignment.Center, HorizontalAlignment = HorizontalAlignment.Right, FontWeight = FontWeights.Bold, DataContext = feature };
            ...
            grid.Children.Add(check); grid.Children.Add(textStack); grid.Children.Add(status);
            FeaturePanel.Children.Add(grid); FeaturePanel.Children.Add(new Separator());
        }
    }

    private async Task RefreshStatusAsync()
    {
        ActionStatusText.Text = "상태 확인 중...";
        await _slimService.RefreshAsync(_features);
        UpdateCounters();
        ActionStatusText.Text = "현재 Windows 설정을 확인했습니다.";
    }

    private void UpdateCounters()
    {
        TotalCountText.Text = $"{_features.Count}개";
        ActiveCountText.Text = $"{_features.Count(x => x.State == FeatureState.Enabled)}개";
        InactiveCountText.Text = $"{_features.Count(x => x.State == FeatureState.Disabled)}개";
    }
    ...
    {
        var count = _features.Count(x => x.IsSelected);
        if (count == 0) { ActionStatusText.Text = "선택된 항목이 없습니다."; return; }
        ActionStatusText.Text = $"{count}개 항목 비활성화 적용 중...";
        await _slimService.ApplyAsync(_features, disable: true, LogService.AppendSlimLog);
        await RefreshStatusAsync();
        ActionStatusText.Text = $"{count}개 항목 처리 완료 · 일부 변경은 다음 로그인/재부팅 후 완전히 적용될 수 있습니다.";
    }
    ...
    private async void RestoreSelected_Click(object sender, RoutedEventArgs e)
    {
        var count = _features.Count(x => x.IsSelected);
        if (count == 0) { ActionStatusText.Text = "선택된 항목이 없습니다."; return; }
        ActionStatusText.Text = $"{count}개 항목 복구 중...";
        await _slimService.ApplyAsync(_features, disable: false, LogService.AppendSlimLog);
        await RefreshStatusAsync();
        ActionStatusText.Text = $"{count}개 항목 복구 처리 완료 · 일부 변경은 다음 로그인/재부팅 후 완전히 적용될 수 있습니다.";
    }
    ...
    private async void RunInspection_Click(object sender, RoutedEventArgs e)
    {
        try
        {
            InspectionButton.IsEnabled = false;
            InspectionStatusText.Text = "검사 중...";
            InspectionProgress.IsIndeterminate = true;
            InspectionElapsedText.Text = "0초";
            _inspectionWatch.Restart(); _inspectionTimer.Start();
            var path = await _inspector.RunAsync();
            _inspectionWatch.Stop(); _inspectionTimer.Stop();
            InspectionProgress.IsIndeterminate = false; InspectionProgress.Value = 100;
            InspectionElapsedText.Text = $"{(int)_inspectionWatch.Elapsed.TotalSeconds}초";
            InspectionStatusText.Text = $"검사 완료 · {System.IO.Path.GetFileName(path)}";
        }
        catch (Exception ex)
        {
            _inspectionWatch.Stop(); _inspectionTimer.Stop();
            InspectionProgress.IsIndeterminate = false; InspectionProgress.Value = 0;
